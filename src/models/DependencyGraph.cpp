...
void DependencyGraph::replace_dependents(std::string first_par, std::vector<std::string> vector_dependents)
{
    std::unordered_map<std::string, std::unordered_set<std::string>>::iterator found_dependents = dependents.find(first_par);  

                if(found_dependents != dependents.end())
                {
                  std::unordered_set<std::string> copy_of_dependents = found_dependents->second;
                  for(const std::string& i : copy_of_dependents) 
                  {
                      DependencyGraph:remove_dependency(first_par, i);
                  }
                }

                 for(const std::string& j : vector_dependents) 
                  {
                      DependencyGraph:add_dependency(first_par, j);
                  }

}

void DependencyGraph::replace_dependees(std::string first_par, std::vector<std::string> vector_dependees)
{
    std::unordered_map<std::string, std::unordered_set<std::string>>::iterator found_dependees = dependees.find(first_par);

       if(found_dependees != dependents.end())
                {
                  std::unordered_set<std::string> copy_of_dependents = found_dependees->second;
                  for(const std::string& i : copy_of_dependents) 
                  {
                      DependencyGraph:remove_dependency(first_par, i);
                  }
                 ...
                 for(const std::string& j : vector_dependees) 
                  {
                      DependencyGraph:add_dependency(first_par, j);
                  }
